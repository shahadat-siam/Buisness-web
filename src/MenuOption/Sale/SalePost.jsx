  ...
  return (
    <div >
      ...
      {salesData.map((sale, idx) => (
        <div key={idx} className="bg-white border shadow-sm rounded-md p-2 space-y-3 text-sm text-gray-800 w-full relative">
          <div className="flex justify-between  border-b pb-2">
            <div>
              <p className="">#{sale.invoiceNo}</p>
              <p className=' '>Delevered to <span className='font-bold uppercase'>{sale.customer}</span>  ,   {sale.project} </p> 
              <p className="text-sm text-gray-500">{sale.date},  {sale.showroom}</p> 
            </div>
